...
var Prerendering = /** @class */ (function () {
    ...
    Prerendering.prototype.start = function () {
        return __awaiter(this, void 0, void 0, function () {
            var start, browser, duration, _loop_1, this_1, _i, _a, route;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        if (this.inDebugMode()) {
                            fancy_log_1.info("opening Chrome...");
                        }
                    ...
                    case 1:
                        ...
                        if (this.inDebugMode()) {
                            fancy_log_1.info("opened Chrome (" + duration + ")");
                        }
                        _loop_1 = function (route) {
                            var routeUrl, coloredRouteUrl, contentPath, timeout, page, exception_1, selectors, selectorsToWaitFor, exception_2, html, bytes;
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0:
                                        routeUrl = this_1.getBaseUrl() + route.getUrl();
                                        coloredRouteUrl = cliColor.yellow(routeUrl);
                                        contentPath = this_1.getFolderPath() + "/" + route.getUrl() + "/index.html";
                                        timeout = this_1.gettimeout();
                                        if (this_1.inDebugMode()) {
                                            fancy_log_1.info("opening a tab...");
                                        }
                                        start = new Date().getTime();
                                    ...
                                    case 1:
                                        ...
                                        duration = cliColor.blackBright(prettyMs(new Date().getTime() - start));
                                        if (this_1.inDebugMode()) {
                                            fancy_log_1.info("opened a tab (" + duration + ")");
                                            fancy_log_1.info("navigating to " + coloredRouteUrl + "...");
                                        }
                                        start = new Date().getTime();
                                        _a.label = 2;
                                    case 2:
                                    ...
                                    case 4:
                                        exception_1 = _a.sent();
                                        if (this_1.inDebugMode()) {
                                            fancy_log_1.error(exception_1);
                                        }
                                        return [2 /*return*/, "continue"];
                                    case 5:
                                        duration = cliColor.blackBright(prettyMs(new Date().getTime() - start));
                                        if (this_1.inDebugMode()) {
                                            fancy_log_1.info("navigated to " + coloredRouteUrl + " (" + duration + ")");
                                        }
                                        if (!route.hasSelectorsToWaitFor()) return [3 /*break*/, 10];
                                        selectors = route.getSelectorsToWaitFor();
                                        if (this_1.inDebugMode()) {
                                            fancy_log_1.info("waiting for " + selectors.length + " selectors...");
                                        }
                                        start = new Date().getTime();
                                    ...
                                    case 8:
                                        exception_2 = _a.sent();
                                        if (this_1.inDebugMode()) {
                                            fancy_log_1.error(exception_2);
                                        }
                                        return [2 /*return*/, "continue"];
                                    case 9:
                                        duration = cliColor.blackBright(prettyMs(new Date().getTime() - start));
                                        if (this_1.inDebugMode()) {
                                            fancy_log_1.info("found the selectors (" + duration + ")");
                                        }
                                        _a.label = 10;
                                    case 10:
                                        if (this_1.inDebugMode()) {
                                            fancy_log_1.info("copying the HTML...");
                                        }
                                        start = new Date().getTime();
                                        return [4 /*yield*/, page.content()];
                                    case 11:
                                        html = _a.sent();
                                        bytes = prettyBytes(html.length);
                                        duration = cliColor.blackBright(prettyMs(new Date().getTime() - start));
                                        if (this_1.inDebugMode()) {
                                            fancy_log_1.info("copied " + bytes + " of HTML (" + duration + ")");
                                            fancy_log_1.info("saving to file...");
                                        }
                                        start = new Date().getTime();
                                        mkdirp.sync(path_1.dirname(contentPath));
                                        fs_2.writeFileSync(contentPath, html);
                                        duration = cliColor.blackBright(prettyMs(new Date().getTime() - start));
                                        if (this_1.inDebugMode()) {
                                            fancy_log_1.info("saved the file (" + duration + ")");
                                        }
                                        return [2 /*return*/];
                                }
                            });
                    ...
                    case 5:
                        if (this.inDebugMode()) {
                            fancy_log_1.info("closing Chrome...");
                        }
                        start = new Date().getTime();
                    ...
                    case 6:
                        ...
                        duration = cliColor.blackBright(prettyMs(new Date().getTime() - start));
                        if (this.inDebugMode()) {
                            fancy_log_1.info("closed Chrome (" + duration + ")");
                        }
